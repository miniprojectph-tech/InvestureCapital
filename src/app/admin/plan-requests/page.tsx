...
import { Check, X, Clock, AlertCircle, Loader2, Coins, Receipt, ExternalLink, ImageOff, UserPlus, Search } from "lucide-react";
...
export default function AdminPlanRequestsPage() {
  ...
  async function approve(r: PlanRequest) {
    if (!user?.isAdmin) return;
    if (!r.termMonths) {
      setError("This is a legacy request without a term — reject it and ask the member to re-submit.");
      return;
    }
    setBusyId(r.id);
    setError(null);
    ...
    try {
      ...
      setNotice(
        `Activated ${res.placementId} for ${r.userName}: ${res.cycles} payouts of ${peso(res.perCycle)}` +
          ...
          ` · ${res.commissionsPaid} upline commission${res.commissionsPaid === 1 ? "" : "s"} paid` +
          (res.fastStartPaid.length ? ` · Fast-Start paid (${res.fastStartPaid.join(", ")})` : ""),
      );
    } catch (e) {
  ...
  return (
    <div>
      <TopHeader title="Placement requests" subtitle={`${counts.pending} pending · ${counts.approved} approved · ${counts.rejected} rejected · live from Firestore`} />
      ...
      {notice && (
        <div className="mb-3 flex items-start gap-2 px-3 py-2 bg-green/10 border border-green/30 rounded-lg text-[11px] text-green">
      ...
      <Card>
        <CardHeader
          ...
          right={tab === "pending" && counts.pending > 0 ? (<span className="text-[10px] text-vault flex items-center gap-1"><AlertCircle className="w-3 h-3" /> Action needed</span>) : undefined}
        />
        ...
        {!loading && filtered.length === 0 && <p className="text-[11px] text-text-subtle text-center py-8 m-0">Nothing in this tab.</p>}

        {filtered.map((r, i) => {
          const isBusy = busyId === r.id;
          return (
            <div key={r.id} className={cn("flex flex-wrap items-center gap-3 py-3", i < filtered.length - 1 && "border-b border-border")}>
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-[10px] text-text-subtle mt-0.5 m-0 truncate">
                  ...
                  {r.referenceNumber && <span className="text-vault-muted ml-1">· ref {r.referenceNumber}</span>}
                  {r.placementId && <span className="text-green ml-1 font-mono">· {r.placementId}</span>}
              ...
              <button
                ...
                className={cn("w-11 h-11 rounded-md overflow-hidden border flex items-center justify-center shrink-0 transition", r.receiptUrl ? "border-border-strong hover:border-border-gold cursor-zoom-in" : "border-border cursor-not-allowed")}
                aria-label={r.receiptUrl ? "View receipt" : "No receipt"}
              >
                {r.receiptUrl ? (
                  /* eslint-disable-next-line @next/next/no-img-element */
                  <img src={r.receiptUrl} alt="Receipt" className="w-full h-full object-cover bg-white" />
                ) : (
              ...
              {tab === "pending" ? (
                <div className="flex gap-1.5 ml-auto sm:ml-0 w-full sm:w-auto">
                  ...
                  <button onClick={() => reject(r.id)} disabled={isBusy} className="flex-1 sm:flex-none text-[11px] px-3 py-1.5 bg-card-elev border border-border-strong text-text-muted rounded-md hover:text-red hover:border-red/30 transition flex items-center justify-center gap-1.5 disabled:opacity-60">
                    <X className="w-3 h-3" /> Reject
                  </button>
                </div>
              ) : (
                <span className={cn("text-[10px] px-2 py-0.5 rounded-md font-medium", statusMeta[r.status].bg, statusMeta[r.status].color)}>{statusMeta[r.status].label}</span>
              )}
            </div>
          );
        })}
      ...
      <Modal open={viewReceipt !== null} onClose={() => setViewReceipt(null)} title="Payment receipt" maxWidth="max-w-2xl">
        {viewReceipt && (
          <div className="flex flex-col gap-3">
            <div className="flex items-center justify-between text-[11px] text-text-muted">
              <span>{viewReceipt.userName} · {describe(viewReceipt)} · {viewReceipt.methodLabel}</span>
            ...
            <div className="bg-white rounded-lg overflow-hidden flex items-center justify-center max-h-[70vh]">
              {viewReceipt.receiptUrl ? (
                /* eslint-disable-next-line @next/next/no-img-element */
                <img src={viewReceipt.receiptUrl} alt="Receipt" className="max-w-full max-h-[70vh] object-contain" />
              ) : (
                <div className="py-20 text-text-dim">No receipt attached</div>
              )}
            </div>
            {viewReceipt.receiptUrl && (
              <a href={viewReceipt.receiptUrl} target="_blank" rel="noopener noreferrer" className="text-[11px] text-gold hover:underline flex items-center justify-center gap-1.5">
...
function ManualActivateModal({ open, onClose, onError }: { open: boolean; onClose: () => void; onError: (msg: string | null) => void }) {
  const { user } = useAuth();
  ...
  useEffect(() => {
    ...
    listAllUsers(db).then(setUsers).catch(() => setUsers([])).finally(() => setLoadingUsers(false));
  }, [open]);

  useEffect(() => {
    if (open && amount === 0) setAmount(cfg.minPlacement);
  }, [open, cfg.minPlacement, amount]);
  ...
  function close() {
    onClose();
    setTimeout(() => { setSelectedUid(""); setMonths(null); setAmount(0); setDone(null); setSearch(""); }, 250);
  }
  ...
  async function activate() {
    if (!user?.isAdmin || !selectedUid || amountError) return;
    setBusy(true);
    onError(null);
    try {
  ...
  return (
    <Modal open={open} onClose={close} title="Manual placement" maxWidth="max-w-md">
      {done ? (
        <div className="py-6 flex flex-col items-center text-center gap-3">
          ...
          <p className="text-[14px] font-medium m-0">Placement activated</p>
          <p className="text-[11px] text-text-muted m-0">{formatPHP(amount)} · {term.months}-month · {users.find((u) => u.uid === selectedUid)?.name ?? selectedUid}</p>
          <p className="text-[10px] text-text-subtle m-0">{done}</p>
          <button onClick={close} className="mt-2 px-5 py-2 bg-gold text-gold-dark rounded-lg text-[12px] font-medium">Done</button>
        </div>
      ) : (
        <div className="flex flex-col gap-4">
          <div>
            <label className="block text-[10px] text-text-muted uppercase tracking-wider mb-1.5">Select member</label>
            {loadingUsers ? (
              ...
              <>
                <div className="flex items-center gap-2 px-3 py-2 bg-canvas border border-border rounded-lg mb-2">
                  ...
                  <input type="text" value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search by name or email…" className="bg-transparent text-[12px] outline-none w-full text-text placeholder:text-text-subtle" />
                </div>
                <div className="max-h-[160px] overflow-y-auto border border-border rounded-lg">
                  {filteredUsers.map((u) => (
                    <button key={u.uid} onClick={() => setSelectedUid(u.uid)} className={cn("w-full text-left px-3 py-2 text-[11px] flex items-center gap-2 transition border-b border-border last:border-0", selectedUid === u.uid ? "bg-gold/10 text-gold" : "hover:bg-card-elev text-text-muted")}>
                      <div className="w-6 h-6 rounded-full bg-blue/15 text-blue text-[9px] font-medium flex items-center justify-center shrink-0">{(u.name?.[0] ?? "?").toUpperCase()}</div>
                  ...
                  {filteredUsers.length === 0 && <p className="text-[11px] text-text-subtle text-center py-4 m-0">No members found.</p>}
                </div>
              </>
            )}
          </div>
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-1.5">
              {cfg.terms.map((t) => (
                <button key={t.months} onClick={() => setMonths(t.months)} className={cn("text-[11px] px-3 py-1.5 rounded-full border transition", term.months === t.months ? "bg-gold/10 border-border-gold text-gold font-medium" : "bg-card-elev border-border text-text-muted hover:text-text")}>
                  {t.months} month{t.months > 1 ? "s" : ""}{t.lockedBonusPerUnit > 0 ? ` · +${peso(t.lockedBonusPerUnit)}/unit` : ""}
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            <label className="block text-[10px] text-text-muted uppercase tracking-wider mb-1.5">Amount</label>
            <div className="flex items-center gap-2.5 px-3 py-2.5 bg-canvas border border-border rounded-lg focus-within:border-gold/40">
              ...
              <input type="number" step={cfg.increment} min={cfg.minPlacement} value={amount || ""} onChange={(e) => setAmount(Math.max(0, parseInt(e.target.value) || 0))} placeholder="1,000" className="flex-1 bg-transparent text-[18px] font-medium font-mono text-text outline-none tabular-nums" />
            </div>
            {amountError ? (
              <p className="text-[10px] text-red mt-1 m-0">{amountError}</p>
            ) : (
          ...
          <div className="flex gap-2 mt-1">
            <button onClick={close} className="flex-1 py-2.5 border border-border-strong rounded-lg text-[12px] text-text-muted hover:bg-card-elev transition">Cancel</button>
            <button
              onClick={activate}
              disabled={busy || !selectedUid || !!amountError}
              className={cn("flex-1 py-2.5 rounded-lg text-[12px] font-medium flex items-center justify-center gap-2 transition", selectedUid && !amountError ? "bg-gold text-gold-dark hover:brightness-110" : "bg-card-elev text-text-subtle cursor-not-allowed")}
            >
              {busy ? <Loader2 className="w-4 h-4 animate-spin" /> : (<>Activate placement <Check className="w-3.5 h-3.5" /></>)}
            </button>
          </div>
        </div>
      )}
