...
import { startGame, splitJackpot } from "@/lib/tongits-game";
...
import { TongitsIdleOverlay } from "@/components/TongitsIdleOverlay";
...
export default function TongitsRoomPage() {
  ...
  useEffect(() => {
    if (!room || room.status !== "ready" || room.gamesPlayed === 0) return;
    if (autoStartRef.current) return;
    autoStartRef.current = true;
    startGame(code).catch(() => {});
  }, [room?.status, room?.gamesPlayed, code]);
  ...
  if (room.status === "in_game") {
    const amIdle = me?.role === "idle";
    if (hasTableArt && wide) {
      return (
        <>
          <TongitsGameTableArt code={code} room={room} spectating={amIdle} />
          {amIdle && <TongitsIdleOverlay code={code} />}
        </>
      );
    }
    return (
